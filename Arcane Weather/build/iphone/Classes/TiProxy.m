...
//Rare exceptions to indicate a bug in the _arcaneweather code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_arcaneweather = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_arcaneweather]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.arcaneweather.module"
